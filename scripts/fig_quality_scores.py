"""Quality scores distribution and Top 10 papers chart."""
import pandas as pd
import numpy as np
...
df = pd.read_csv('../scores_vqa.csv')

fig, axes = plt.subplots(1, 2, figsize=(14, 5), gridspec_kw={'width_ratios': [1, 1.4]})

# --- Left: Score distribution histogram ---
ax1 = axes[0]
bins = np.arange(5.5, 14, 1)
ax1.hist(df['Total'], bins=bins, color=COLORS['primary'], edgecolor='white',
         linewidth=0.8, alpha=0.85)
ax1.set_xlabel('Quality Score (0–13)')
ax1.set_ylabel('Number of Papers')
ax1.set_title('Score Distribution (VQA, n={})'.format(len(df)))
ax1.set_xticks(range(6, 14))
...
for t in top10['Title']:
    if len(t) > 45:
        short_names.append(t[:42] + '...')
    else:
        short_names.append(t)

y_pos = np.arange(len(top10))
bars = ax2.barh(y_pos, top10['Total'].values, color=COLORS['primary'],
...
ax2.set_title('Top 10 Papers (Score + QP Coverage)')
ax2.set_xlim(0, 15)
...
fig.tight_layout()
fig.savefig(f'{FIGURES_DIR}/fig_quality_scores.pdf')
fig.savefig(f'{FIGURES_DIR}/fig_quality_scores.png')
print('Saved: fig_quality_scores.pdf / .png')
